...
public class UpdateQueryInstanceMessageHandler extends RequestHandler {
	 ...
	 */
	public UpdateQueryInstanceMessageHandler(String requestXml, boolean isMgr, String username)
			throws I2B2Exception {
		isManager = isMgr;
		userId = username;
		try {
	...
	public BodyType execute() {
		...
		try {
			...
			String message = instanceMessageRequestType.getMessage().replaceAll("\\<[^>]*>","");
			QueryInfoBean query = new QueryInfoBean();
			query.setQueryInstanceMessage(this.getDataSourceLookup(),
					instanceId, message, isManager, userId);
