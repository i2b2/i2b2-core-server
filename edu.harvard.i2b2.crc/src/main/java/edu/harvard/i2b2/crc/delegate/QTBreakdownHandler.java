 ...
 * @author rkuttan
 */
public class QTBreakdownHandler extends RequestHandler {
	private QueryDefinitionRequestType getRequestMessageType =
			null;
	 ...
	 */
	public QTBreakdownHandler(String requestXml)
			throws I2B2Exception {
		try {
			this.getRequestMessageType = (QueryDefinitionRequestType) this.getRequestType(requestXml,
	...
	public BodyType execute() throws I2B2Exception {
		...
		CrcXmlResultResponseType masterResponseType = null;
		try {
			...
			masterResponseType = query.getQTBreakdownForAdmin(getDataSourceLookup(),userRequestType, getRequestMessageType.getResultOutputList());
			...
			masterResponseType = new CrcXmlResultResponseType();
