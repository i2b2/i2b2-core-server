...
IQueryResultInstanceDao {
	...
	public int getResultInstanceCountBySetSize(String userId, int compareDays,
        ...
        DAOFactoryHelper.SNOWFLAKE) ) {
			...
			queryCountSql =
				" select count(r1.result_instance_id) result_count, r1.real_set_size "
			+ " from " + this.getDbSchemaName() + "qt_query_result_instance r1 "
			+ " inner join " + this.getDbSchemaName() + "qt_query_result_instance r2 "
			+ "   on r1.real_set_size = r2.real_set_size, "
			+ this.getDbSchemaName() + "qt_query_instance qi "
			+ " where "
			+ "  r1.start_date between DATEADD(day, -" + compareDays + ", CURRENT_TIMESTAMP()) and CURRENT_TIMESTAMP() "
			+ "  and r2.start_date between DATEADD(day, -" + compareDays + ", CURRENT_TIMESTAMP()) and CURRENT_TIMESTAMP() "
			+ "  and r1.result_type_id = ?"
			+ "  and r2.result_type_id = ? "
			+ "  and qi.user_id = ? "
			+ "  and qi.query_instance_id = r1.query_instance_id "
