...
public class SQLServerTranslator extends Translator
{
	...
	public String translate( TokenizedStatement ts )
	{
		...
		{			
			...
			else if (t.getPOS() == Token.POS.NOT)
			{
				if (!translatedStr.endsWith(" "))
					translatedStr = translatedStr + " ";
				translatedStr = translatedStr + "and not";
			}
			...
			else if (t.getPOS() == Token.POS.DOUBLEQUOTE )
			{
				if (hasOpenQuotes)
					translatedStr = translatedStr + t.getString();
				else
				{
					if ((i != 0) && ts.getTokenAt(i-1).getPOS()!= Token.POS.DOUBLEQUOTE && ts.getTokenAt(i-1).getPOS()!= Token.POS.OPEN_PARENTHESIS)
						translatedStr = translatedStr + " ";
					translatedStr = translatedStr + t.getString();
				}
	...
	public static void main( String [] args )
	{
		...
		SQLServerTranslator t = new SQLServerTranslator();
