...
public class AddModifierHandler extends RequestHandler {
	private AddChildDataMessage  addChildMsg = null;
	private ModifierType addChildType = null;
	private ProjectType projectInfo = null;
	
	public AddModifierHandler(AddChildDataMessage requestMsg) throws I2B2Exception{
	...
	public String execute() throws I2B2Exception{
		...
		if(!Roles.getInstance().isRoleValid(projectInfo)){
			...
			try {
				...
				e1.printStackTrace();
				log.error("AddModifierHandler received exception");
		...
		if(responseMessageType == null) {
			...
			if (numAdded == 0) {
				log.error("modifier not inserted");
