...
public class CheckMetadataTableHandler extends RequestHandler {
	private LoadDataMessage  loadMsg = null;
	private MetadataLoadType conceptsType = null;
	private ProjectType projectInfo = null;
	
	public CheckMetadataTableHandler(LoadDataMessage requestMsg) throws I2B2Exception{
	...
	public String execute() throws I2B2Exception{
		...
		if(!Roles.getInstance().isRoleValid(projectInfo)){
			...
			try {
				numFound = 
						persistDao.checkForTableExistence(this.getDbInfo(), loadMsg.getMetadataLoad().getTableName());
			} catch (Exception e1) {
				e1.printStackTrace();
				log.error("CheckMetadataHandler received exception");
		...
		if(responseMessageType == null) {
			...
			if (numFound > 0) {
				log.error("Metadata table exists");
				responseMessageType = MessageFactory.doBuildErrorResponse(loadMsg.getMessageHeaderType(), "Previously installed");
			}
			else if (numFound == -1) {
